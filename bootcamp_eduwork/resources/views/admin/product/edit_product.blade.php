        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div style="background: #fff; border-radius: 16px; box-shadow: 0 4px 20px rgba(108,92,231,0.08); overflow: hidden;">
                ...
                <div style="padding: 24px 32px; background: linear-gradient(135deg, #6C5CE7, #A29BFE); color: #fff;">
                    <h3 style="margin: 0; font-size: 1.15rem; font-weight: 700;">✏️ Edit Produk</h3>
                    <p style="margin: 6px 0 0 0; font-size: 0.85rem; opacity: 0.85;">Ubah data produk "{{ $product->name }}"</p>
                </div>

                {{-- Form --}}
                <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" style="padding: 32px;">
                    @csrf
                    @method('PUT')
                    ...
                    <div style="margin-bottom: 24px;">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" placeholder="Masukkan nama produk"
                    ...
                    <div style="margin-bottom: 24px;">
                        ...
                        <textarea name="description" id="description" rows="4" placeholder="Masukkan deskripsi produk"
                                  ...
                                  onfocus="this.style.borderColor='#A29BFE'"
                                  onblur="this.style.borderColor='#f1f1f1'">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <p style="margin-top: 6px; font-size: 0.8rem; color: #E17055;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
                        <div>
                            ...
                            <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}" placeholder="0" min="0"
                                   style="width: 100%; padding: 12px 16px; border: 2px solid #f1f1f1; border-radius: 10px; font-size: 0.95rem; font-family: 'Inter', sans-serif; transition: border-color 0.2s; outline: none; box-sizing: border-box;"
                                   onfocus="this.style.borderColor='#A29BFE'"
                                   onblur="this.style.borderColor='#f1f1f1'">
                            @error('stock')
                        ...
                        <div>
                            ...
                            <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" placeholder="0" min="0"
                                   style="width: 100%; padding: 12px 16px; border: 2px solid #f1f1f1; border-radius: 10px; font-size: 0.95rem; font-family: 'Inter', sans-serif; transition: border-color 0.2s; outline: none; box-sizing: border-box;"
                                   onfocus="this.style.borderColor='#A29BFE'"
                                   onblur="this.style.borderColor='#f1f1f1'">
                            @error('price')
                    ...
                    {{-- Gambar Saat Ini --}}
                    <div style="margin-bottom: 24px;">
                        <label style="display: block; font-weight: 600; font-size: 0.9rem; color: #2D3436; margin-bottom: 8px;">
                            Gambar Saat Ini
                        </label>
                        @if($product->image)
                            <div style="display: inline-block; border-radius: 10px; overflow: hidden; border: 2px solid #f1f1f1;">
                                <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}"
                                     style="width: 120px; height: 120px; object-fit: cover; display: block;">
                            </div>
                        @else
                            <div style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 16px; background: #F8F9FD; border-radius: 10px; color: #B2BEC3; font-size: 0.9rem;">
                                📷 Belum ada gambar
                            </div>
                        @endif
                    </div>
                    ...
                    <div style="margin-bottom: 32px;">
                        ...
                        <div style="border: 2px dashed #d1d1d1; border-radius: 10px; padding: 24px; text-align: center; transition: border-color 0.2s; cursor: pointer;"
                            ...
                            <p style="margin: 0; font-size: 0.9rem; color: #636E72;">Klik untuk upload gambar baru</p>
