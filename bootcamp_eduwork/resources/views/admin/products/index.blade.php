        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div style="background: #fff; border-radius: 16px; box-shadow: 0 4px 20px rgba(108,92,231,0.08); overflow: hidden;">
                <div style="overflow-x: auto;">
                    <table style="width: 100%; border-collapse: collapse; font-family: 'Inter', sans-serif; min-width: 800px;">
                        <thead>
                            <tr style="background: linear-gradient(135deg, #6C5CE7, #A29BFE); color: #fff;">
                                ...
                                <th style="padding: 14px 20px; text-align: left; font-weight: 600; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px;">Gambar</th>
                                <th style="padding: 14px 20px; text-align: left; font-weight: 600; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px;">Nama</th>
                                <th style="padding: 14px 20px; text-align: left; font-weight: 600; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px;">Deskripsi</th>
                                <th style="padding: 14px 20px; text-align: center; font-weight: 600; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px;">Stok</th>
                                <th style="padding: 14px 20px; text-align: right; font-weight: 600; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px;">Harga</th>
                        ...
                        <tbody>
                            @forelse($products as $product)
                                <tr style="border-bottom: 1px solid #f1f1f1; transition: background 0.15s;"
                                    ...
                                    <td style="padding: 14px 20px; font-size: 0.9rem; color: #636E72;">{{ $product->id }}</td>
                                    <td style="padding: 14px 20px;">
                                        @if($product->image)
                                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}"
                                                 style="width: 60px; height: 60px; object-fit: cover; border-radius: 10px; border: 2px solid #f1f1f1;">
                                        @else
                                            <div style="width: 60px; height: 60px; background: #F8F9FD; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; border: 2px solid #f1f1f1;">
                                                📷
                                            </div>
                                        @endif
                                    </td>
                                    <td style="padding: 14px 20px; font-size: 0.95rem; font-weight: 500; color: #2D3436;">{{ $product->name }}</td>
                                    <td style="padding: 14px 20px; font-size: 0.85rem; color: #636E72; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                        {{ Str::limit($product->description, 50) }}
                                    </td>
                                    <td style="padding: 14px 20px; text-align: center;">
                                        <span style="display: inline-block; background: {{ $product->stock > 0 ? 'linear-gradient(135deg, #00b894, #55efc4)' : 'linear-gradient(135deg, #e17055, #fab1a0)' }}; color: #fff; padding: 4px 14px; border-radius: 20px; font-size: 0.85rem; font-weight: 600;">
                                            {{ $product->stock }}
                                        </span>
                                    </td>
                                    <td style="padding: 14px 20px; text-align: right; font-size: 0.95rem; font-weight: 600; color: #6C5CE7;">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" style="padding: 40px 20px; text-align: center; color: #B2BEC3; font-size: 1rem;">
